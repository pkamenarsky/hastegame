...
namespace corona {
  ...
  Image* OpenTGA(File* file) {
    ...
    if ((image_type != 2 && image_type != 10) || (pixel_depth != 24 && pixel_depth != 32)) {
      return 0;
    }
		...
		case 3: // 24 bit
			case 4: // 32 bit
				for (i=m_Width*m_Height;i>0;) {
					src.read(&rlebyte,sizeof(rlebyte));
					if (rlebyte&0x80) {
						rlebyte&=0x7F;
						++rlebyte;
						i-=rlebyte;
						...
						dummyl=0xFF000000;
						src.read(&dummyl,m_BytesPP);
						dummyl=(dummyl&0xff00ff00)|((dummyl&0xff)<<16)|((dummyl&0xff0000)>>16);
						while (rlebyte>0) {
							pix << dummyl;
							--rlebyte;
						}
		...
		if (pixel_depth == 24) {
			...
			for (int i=width*height;i>0;) {
				...
				if (rlebyte&0x80) {
					...
					file->read(pPixel,3*((int)rlebyte));
					pPixel+=3*((int)rlebyte);
				}
			}
		} else if (pixel_depth == 32) {
			format = PF_B8G8R8A8;
			...
			for (int i=width*height;i>0;) {
				...
				if (rlebyte&0x80) {
					...
					file->read(pPixel,4*((int)rlebyte));
					pPixel+=4*((int)rlebyte);
				}
			}
		}
	} else return 0;
