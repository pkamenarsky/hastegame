...
#include "fmodaudioserver.hh"
#include "fmodsound.hh"

namespace ion {
namespace fmoddrv {

	const char* FmodAudioserver::errorResultDesc(const FMOD_RESULT result)
	{
		switch (result) {
			...
			case FMOD_ERR_INTERNAL: return "Internal FMOD error: an error occured that wasnt supposed to. Contact support"; 
	...
	void FmodAudioserver::initialize(const ion_uint32 maxVoices)
	{
		m_pSystem->init(maxVoices,FMOD_INIT_NORMAL,0);
	}

	void FmodAudioserver::update()
	{
		m_pSystem->update();
	}

	FmodSound* FmodAudioserver::createSound(base::Streamable& streamable,const audio::Sound::Looptype looptype,
		const bool isStream,const bool is3DSound)
	{
		if (!streamable.streamIsOk()) return 0;

		base::Datablock *pDBlock= ::new base::Datablock;
		pDBlock->transferFrom(streamable);

		FMOD_CREATESOUNDEXINFO exinfo;
		memset(&exinfo,0,sizeof(exinfo));
		exinfo.cbsize=sizeof(exinfo);
		exinfo.length=pDBlock->size();

		FMOD_MODE mode=FMOD_OPENMEMORY|(is3DSound ? FMOD_3D : FMOD_2D);
		if (isStream) mode|=(FMOD_CREATESTREAM|FMOD_LOOP_OFF); // Streams do not allow looping
		else {
			switch (looptype) {
				case audio::Sound::NoLoop:mode|=FMOD_LOOP_OFF; break;
				case audio::Sound::NormalLoop:mode|=FMOD_LOOP_NORMAL; break;
				case audio::Sound::BidiLoop:mode|=FMOD_LOOP_BIDI; break;
			};
		}

		FMOD::Sound *pSound=0;
		FMOD_RESULT ret=m_pSystem->createSound((const char *)(pDBlock->data()),mode,&exinfo,&pSound);

		if (ret!=FMOD_OK) {
			base::log("FmodAudioserver::createSound()",base::Error) << errorResultDesc(ret) << "\n";
			::delete pDBlock;
			return 0;
		}

		return new FmodSound(m_pSystem,pSound,pDBlock);
	}

	FmodSound* FmodAudioserver::createSoundFromLocal(const base::String& filename,const audio::Sound::Looptype looptype,
		const bool isStream,const bool is3DSound)
	{
		FMOD_MODE mode=(is3DSound ? FMOD_3D : FMOD_2D);
		if (isStream) mode|=(FMOD_CREATESTREAM|FMOD_LOOP_OFF); // Streams do not allow looping
		else {
			switch (looptype) {
				case audio::Sound::NoLoop:mode|=FMOD_LOOP_OFF; break;
		...
		FMOD::Sound *pSound=0;
		m_pSystem->createSound(filename.cstr(),mode,0,&pSound);

		return new FmodSound(m_pSystem,pSound,0);
	}

	void FmodAudioserver::listenerAttributes(const math::Vector3f& newposition,const math::Vector3f& newvelocity)
	{
		FMOD_VECTOR pos,vel;
		pos.x=newposition.x();
		pos.y=newposition.y();
		...
		m_pSystem->set3DListenerAttributes(0,&pos,&vel,0,0);
	}

	bool FmodAudioserver::isValid() const
	{
		return (m_pSystem!=0);
	}
...
#endif
ion::audio::Audioserver* createAudioserverInstance()
{
	return new ion::fmoddrv::FmodAudioserver;
}
...
__declspec( dllexport )
#endif
const char* audioserverDescription()
{
	return "FMOD-ex 4.03 audio server build 2006-04-30 1:00";
}
