	...
	void loadOBJ(base::Streamable &streamable,Mesh &rMesh,const float scale,const bool useTangentspace)
	{
		...
		while (!dblock.eof()) {
			...
			if (keyword=="#") continue;
				...
				//phil begin
				ion_uint32 nt, firstv[32], firstt[32], firstn[32];

				for(nt = 0, sstr >> data; data.find_first_of('/') != std::string::npos && nt < 32; sstr >> data, nt++)
				{
					size_t pos1=0,pos2=0;
					pos1=data.find_first_of('/');
					pos2=data.find_last_of('/');
					...
					//ion_uint32 geomvtx,texvtx,normvtx;
					std::stringstream sx(data.substr(0,pos1)); sx >> firstv[nt];
					std::stringstream sy(data.substr(pos1+1,pos2-pos1-1)); sy >> firstt[nt];
					std::stringstream sz(data.substr(pos2+1)); sz >> firstn[nt];
				}

				for (int i=2;i<(int)nt;++i) {
					math::Meshconv::InTriangle tri;

					tri.m_XYZ[0]=firstv[0]-1;
					tri.m_Texcoords2D[0]=firstt[0]-1;
					tri.m_Normals[0]=firstn[0]-1;
					
					tri.m_XYZ[1]=firstv[i-1]-1;
					tri.m_Texcoords2D[1]=firstt[i-1]-1;
					tri.m_Normals[1]=firstn[i-1]-1;

					tri.m_XYZ[2]=firstv[i]-1;
					tri.m_Texcoords2D[2]=firstt[i]-1;
					tri.m_Normals[2]=firstn[i]-1;
