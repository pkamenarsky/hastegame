...
EntWeaponGrenadelauncher::EntWeaponGrenadelauncher(EngineExport *EE) : Entity(EE)
{
...
EntWeaponGrenadelauncher::EntWeaponGrenadelauncher()
{
	EntWeaponGrenadelauncher(NULL);
}
...
void EntWeaponGrenadelauncher::Init()
{
	y = origin[1];
	test = 0;
...
void EntWeaponGrenadelauncher::Think(void)
{
	Entity::Think();
	...
	if(!commands.empty())
		if(*commands.begin() == "+attack" && parent && time <= 0)
		{
			...
			if(rockets > 0)
			{
				...
				ent = EE.SpawnEntity("ent_grenade", 0);
	...
	if(parent)
	{
		...
		Vector3f	na(a[0], a[1] + 160, a[2]), vel = parent->GetVelocity(), v;
...
/*		for(i = 0; i < 3; i++)
			if(v[i] > 0.1)
				view_velocity[i] = min(5, view_velocity[i]++);
			else if(v[i] < -0.1)
				view_velocity[i] = max(-5, view_velocity[i]--);
			else
				view_velocity[i] = 0;
*/
...
void EntWeaponGrenadelauncher::Touch(Entity &other)
{
...
EXPORT_ENTITY(EntWeaponGrenadelauncher)
